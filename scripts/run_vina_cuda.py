#!/usr/bin/env python
"""
Step 4f: Run Vina-CUDA on symmetry-corrected receptors.

Vina-CUDA is an independent GPU reimplementation of AutoDock Vina that
accepts the standard Vina PDBQT inputs and writes standard ``MODEL`` /
``ENDMDL`` output.

Usage:
    python run_vina_cuda.py \
        --receptor-dir runs-n-poses-datasets/symmetry_receptors_pdbqt \
        --ligand-dir runs-n-poses-datasets/symmetry_ligands_pdbqt \
        --output-dir runs-n-poses-datasets/vina_cuda_symmetry \
...
VINA_CUDA = "/home/rquiroga/github/Vina-CUDA/Vina-CUDA_v1.1/Vina-CUDA_v1.1"
CUDA_DIR = "/home/rquiroga/github/Vina-CUDA/Vina-CUDA_v1.1"
...
def run_vina_cuda(receptor_file, ligand_file, output_dir, sys_id, lig_chain,
    ...
    # Box from ligand coordinates (smaller, more reliable on Vina-CUDA)
    box = compute_autobox(parse_pdb_coordinates(ligand_file), margin=margin)
    if box is None:
        return False
    cx, cy, cz, sx, sy, sz = box

    # Vina-CUDA sometimes fails on huge thread counts depending on the GPU.
    # Try the requested thread count, then fall back to smaller values.
    for attempt_threads in (threads, max(threads // 2, 1024), max(threads // 4, 512)):
        cmd = [
            VINA_CUDA,
            ...
            "--out", str(out_pdbqt),
            "--num_modes", "10",
            "--thread", str(attempt_threads),
        ]

        try:
            start = time.time()
            result = subprocess.run(cmd, capture_output=True, text=True,
                                    timeout=900, cwd=CUDA_DIR)
            ...
            with open(runtime_file, "w") as fh:
                json.dump({"method": "vina-cuda-1.1", "threads": attempt_threads,
                           "returncode": result.returncode,
                           "runtime_seconds": elapsed}, fh)

            if result.returncode == 0 and out_pdbqt.exists() and out_pdbqt.stat().st_size > 0:
                return True
            # Don't try smaller thread counts if Vina-CUDA just failed for non-resource reasons.
            break
...
def main():
    parser = argparse.ArgumentParser(description="Run Vina-CUDA on symmetry-corrected receptors")
    ...
    for sys_id in system_ids:
        ...
        for lf in ligand_files:
            ...
            if run_vina_cuda(str(receptor_file), str(lf), str(output_dir),
                             sys_id, lig_chain, threads=args.threads, margin=args.margin):
                ok += 1
                sys_ok = True
            else:
                print(f"  FAIL vina-cuda: {sys_id} {lig_chain}")
                fail += 1

        if not sys_ok:
            fail += 1

    print(f"Vina-CUDA: OK={ok}, Skip={skip}, Fail={fail}")
