...
def run_qvina2(receptor_file, ligand_file, output_dir, sys_id, lig_chain,
               cpu=8, margin=10.0):
    ...
    try:
        ...
        with open(runtime_file, "w") as fh:
            json.dump({"method": "qvina2", "returncode": result.returncode,
...
def main():
    parser = argparse.ArgumentParser(description="Run QVina2 on symmetry-corrected receptors")
    ...
    for sys_id in system_ids:
        ...
        for lf in ligand_files:
            ...
            if run_qvina2(str(receptor_file), str(lf), str(output_dir),
                          sys_id, lig_chain, cpu=args.cpu):
                ok += 1
                sys_ok = True
            else:
                print(f"  FAIL qvina2: {sys_id} {lig_chain}")
                fail += 1

        if not sys_ok:
            fail += 1

    print(f"QVina2: OK={ok}, Skip={skip}, Fail={fail}")
