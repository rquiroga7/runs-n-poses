...
ADGPU = "/home/rquiroga/github/Vina-GPU-2.1/AutoDock-Vina-GPU-2.1/AutoDock-Vina-GPU-2-1"
...
def run_adgpu(receptor_file, ligand_file, output_dir, sys_id, lig_chain,
    ...
    cmd = [
        ...
        "--size_x", str(sx * size_mult), "--size_y", str(sy * size_mult),
        "--size_z", str(sz * size_mult),
        "--out", str(out_pdbqt),
        "--num_modes", "10",
        "--thread", str(threads),
    ]
    ...
    try:
        ...
        with open(runtime_file, "w") as fh:
            json.dump({"method": "ad-gpu-2.1", "threads": threads,
...
def main():
    ...
    for sys_id in system_ids:
        ...
        for lf in ligand_files:
            ...
            if run_adgpu(str(receptor_file), str(lf), str(output_dir),
                         sys_id, lig_chain, threads=args.threads):
                ok += 1
                sys_ok = True
            else:
                print(f"  FAIL adgpu: {sys_id} {lig_chain}")
                fail += 1
