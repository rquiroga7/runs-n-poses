...
# Python executable for RDKit environment (used by fixer scripts)
...
# Path to fixer script that maps docked coordinates onto the GT topology
...
def analyze_system(system_id: str, annotations: pd.DataFrame, run_posebusters_flag: bool = True,
    ...
    for lig_dir in sorted(system_out.iterdir()):
        ...
        if run_posebusters_flag:
            # Build a PB-ready ligand SDF by preserving the docked coordinates
            # and reusing the GT topology. This avoids spurious sanitization or
            # protonation failures caused by PDBT -> SDF conversion.
...
def main():
    ...
    if total_metrics > 0:
        ...
        # Do not print per-row PoseBusters pass statistics here; PoseBusters
        # summaries are computed from the canonical PoseBusters CSV files.
    else:
