#!/usr/bin/env python
"""
Step 4a: Run AutoDock Vina (exhaustiveness=32) on symmetry-corrected receptors.

Adapted from 03_run_vina.py for the symmetry-corrected docking pipeline.

Usage:
    python run_vina_symmetry.py \
        --receptor-dir runs-n-poses-datasets/symmetry_receptors_pdbqt \
        --ligand-dir runs-n-poses-datasets/symmetry_ligands_pdbqt \
        --output-dir runs-n-poses-datasets/autodock_vina_32_symmetry \
...
def run_vina(receptor_file, ligand_file, output_dir, sys_id, lig_chain,
             cpu=8, exhaustiveness=32, margin=10.0):
    """Run Vina docking for a single receptor-ligand pair."""
    ...
    # Clean ROOT tags from receptor (preparation, not timed)
    cleaned = str(receptor_file)
    try:
        with open(receptor_file) as fh:
            if "ROOT" in fh.read():
                cleaned = str(receptor_file).replace(".pdbqt", "_clean.pdbqt")
                if not os.path.exists(cleaned):
                    pdb_tmp = str(receptor_file).replace(".pdbqt", "_roundtrip.pdb")
                    subprocess.run(["obabel-25-07", str(receptor_file), "-O", pdb_tmp],
                                   capture_output=True, text=True, timeout=60)
                    subprocess.run(["obabel-25-07", pdb_tmp, "-O", cleaned, "-h", "-xc", "-xr"],
                                   capture_output=True, text=True, timeout=60)
                    if os.path.exists(pdb_tmp):
                        os.remove(pdb_tmp)
                if not (os.path.exists(cleaned) and os.path.getsize(cleaned) > 0):
                    cleaned = str(receptor_file)
    except Exception:
        cleaned = str(receptor_file)

    box = compute_autobox(parse_pdb_coordinates(cleaned), margin=margin)
    if box is None:
        return False
    cx, cy, cz, sx, sy, sz = box

    # --- Docking (timed precisely) ---
    cmd = [
        VINA,
        "--receptor", cleaned,
    ...
    try:
        ...
        with open(runtime_file, "w") as fh:
            json.dump({"method": "vina", "exhaustiveness": exhaustiveness,
...
def main():
    parser = argparse.ArgumentParser(description="Run Vina on symmetry-corrected receptors")
    ...
    for sys_id in system_ids:
        ...
        for lf in ligand_files:
            ...
            if run_vina(str(receptor_file), str(lf), str(output_dir),
                        sys_id, lig_chain, cpu=args.cpu, exhaustiveness=args.exhaustiveness):
                ok += 1
                sys_ok = True
            else:
                print(f"  FAIL vina: {sys_id} {lig_chain}")
                fail += 1

        if not sys_ok:
            fail += 1

    print(f"Vina: OK={ok}, Skip={skip}, Fail={fail}")
