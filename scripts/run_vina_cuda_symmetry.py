...
def run_cuda(receptor_file, ligand_file, output_dir, sys_id, lig_chain,
             threads=8000, margin=10.0):
    ...
    box = compute_autobox(parse_pdb_coordinates(receptor_file), margin=margin)
    if box is None:
        return False
    cx, cy, cz, sx, sy, sz = box

    for attempt_threads in [threads, threads // 2, threads // 4]:
        cmd = [
            VINA_CUDA,
...
def main():
    ...
    for sys_id in system_ids:
        ...
        for lf in ligand_files:
            ...
            if run_cuda(str(receptor_file), str(lf), str(output_dir),
                        sys_id, lig_chain, threads=args.threads):
                ok += 1
                sys_ok = True
            else:
