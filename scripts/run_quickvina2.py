#!/usr/bin/env python
"""
Step 4d: Run QuickVina2 (scip-qvina) on symmetry-corrected receptors.

QuickVina2 ("scip-qvina") is a successor to QuickVina with improved search
heuristics. It accepts the same Vina-style PDBQT inputs and adds a
``--buffer`` argument that is mandatory for the docking box.

Usage:
    python run_quickvina2.py \
        --receptor-dir runs-n-poses-datasets/symmetry_receptors_pdbqt \
        --ligand-dir runs-n-poses-datasets/symmetry_ligands_pdbqt \
        --output-dir runs-n-poses-datasets/quickvina2_symmetry \
...
QUICKVINA2 = "/home/rquiroga/.local/bin/QuickVina2"
...
def run_quickvina2(receptor_file, ligand_file, output_dir, sys_id, lig_chain,
    ...
    # QuickVina2 requires a buffer of 0 to use a manual box.
    cmd = [
        QUICKVINA2,
    ...
    try:
        ...
        with open(runtime_file, "w") as fh:
            json.dump({"method": "quickvina2", "exhaustiveness": exhaustiveness,
...
def main():
    parser = argparse.ArgumentParser(description="Run QuickVina2 on symmetry-corrected receptors")
    ...
    for sys_id in system_ids:
        ...
        for lf in ligand_files:
            ...
            if run_quickvina2(str(receptor_file), str(lf), str(output_dir),
                              sys_id, lig_chain, cpu=args.cpu,
                              exhaustiveness=args.exhaustiveness, margin=args.margin):
                ok += 1
                sys_ok = True
            else:
                print(f"  FAIL quickvina2: {sys_id} {lig_chain}")
                fail += 1

        if not sys_ok:
            fail += 1

    print(f"QuickVina2: OK={ok}, Skip={skip}, Fail={fail}")
